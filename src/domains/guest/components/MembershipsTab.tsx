...
export const MembershipsTab: React.FC<MembershipsTabProps> = ({ addAuditLog }) => {
  ...
  React.useEffect(() => {
    const fetchMembers = async () => {
      try {
        const res = await api.guests.listMembership();
        if (res?.success && res.data?.items?.length > 0) {
          const apiMembers = res.data.items.map(g => ({
            id: g.id,
            guest: `${g.firstName} ${g.lastName}`,
            email: g.email,
            phone: g.phone || 'N/A',
            loyaltyPoints: g.loyaltyPoints || 0,
            tier: g.membershipTier || 'GOLD',
            vip: g.vip || false,
            nationality: g.nationality || 'XX',
            preferences: g.preferences || 'None'
          }));
          setMembers(apiMembers);
          if (apiMembers.length > 0) {
            setSelectedId(apiMembers[0].id);
          }
        }
      } catch (err) {
        console.warn('Backend unavailable, using mock members');
      }
    };
    fetchMembers();
  }, []);
