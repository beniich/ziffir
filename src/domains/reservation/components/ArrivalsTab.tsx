...
export const ArrivalsTab: React.FC<ArrivalsTabProps> = ({ flights, userRole = 'operator' }) => {
  ...
  // Real reservations data from API
  ...
  useEffect(() => {
    const fetchArrivals = async () => {
      ...
      try {
        const res = await api.reservations.todayArrivals();
        if (res?.success) {
          const mapped = res.data.items.map((r: any) => ({
            name: `${r.guest.firstName} ${r.guest.lastName}`,
            vip: 'Elite',
            status: r.status,
            info: `Reservation ${r.reference}`,
            flight: 'Private Jet',
          }));
          setApiGuests(mapped);
        }
      } catch (err) {
        console.warn('API reservations not available, falling back to mock data');
      } finally {
  ...
  const displayedGuests = apiGuests.length > 0 ? apiGuests : [
    { name: 'Dr. Al-Fayed', vip: 'Presidential', status: 'Landed', info: 'Connecting Helitender', flight: 'BA245' },
    { name: 'Mr. & Mrs. Rossi', vip: 'Elite', status: 'En Route', info: 'ETA 45 mins', flight: 'AF015' },
    { name: 'Ms. Chen', vip: 'Elite', status: 'Pre-Flight', info: 'Boarding DXB', flight: 'EK201' },
  ];
