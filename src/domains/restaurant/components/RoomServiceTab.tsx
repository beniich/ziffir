...
export const RoomServiceTab: React.FC<RoomServiceTabProps> = ({ addAuditLog, language }) => {
  ...
  const loadOrders = async () => {
    setLoading(true);
    try {
      const res = await api.restaurant.listOrders();
      if (res?.success) {
        // Map backend orders to frontend format
        const mapped = res.data.items.map((o: any) => ({
          id: o.id,
          guest: o.guest ? `${o.guest.firstName} ${o.guest.lastName}` : 'Guest',
          room: o.room?.number ? `Room ${o.room.number}` : o.roomId || 'Unknown',
          details: o.items.map((i: any) => `${i.quantity}x ${i.name}`).join(', ') || 'No details',
          status:
            o.status === 'PENDING' ? 'Preparation' :
            o.status === 'PREPARING' ? 'Quality Check' :
            o.status === 'READY' ? 'Out for Delivery' :
            o.status === 'DELIVERED' ? 'Delivered' : 'Preparation',
          imgUrl: ''
        }));
        setApiOrders(mapped);
      }
  ...
  const combinedOrders = apiOrders.length > 0 ? [...apiOrders, ...localOrders] : [...localOrders];
  ...
  const handleAdvanceStatus = async (id: string) => {
    ...
    const isApiOrder = apiOrders.some(o => o.id === id);
    let nextStatus = '';
    
    if (order.status === 'Preparation') nextStatus = 'PREPARING';
    else if (order.status === 'Quality Check') nextStatus = 'READY';
    else if (order.status === 'Out for Delivery') nextStatus = 'DELIVERED';
    else if (order.status === 'Delivered') nextStatus = 'PENDING';
    
    if (isApiOrder && nextStatus) {
      try {
        await api.restaurant.updateOrderStatus(id, nextStatus);
      ...
      setLocalOrders(prev => prev.map(o => {
        ...
        return { ...o, status: statusMap[o.status] || 'Preparation' };
      }));
      confetti({ particleCount: 15, spread: 35, colors: ['#c19a6b', '#ffffff'] });
    }
    ...
    if (addAuditLog) {
      addAuditLog(
        ...
        `Milestone transition executed for order ${id}. Room alignment: ${order.room}.`,
        'AUTHORIZED'
      );
    }
  };
  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    try {
      // Real API create order
      const apiRes = await api.restaurant.createOrder({
        type: 'ROOM_SERVICE',
        roomId: formSuite,
        guestId: formGuest,
        items: [{ menuItemId: 'custom', quantity: 1, notes: formDetails }],
        specialInstructions: formDetails
      });

      if (apiRes.success) {
        await loadOrders(); // reload from backend
      }
    } catch (err) {
      console.warn('Backend unavailable for createOrder, using local state fallback');
      ...
      const newOrder: RoomServiceOrder = {
        ...
        room: formSuite,
        details: formDetails,
        status: formStatus as RoomServiceOrder['status'],
        imgUrl: ''
      };
